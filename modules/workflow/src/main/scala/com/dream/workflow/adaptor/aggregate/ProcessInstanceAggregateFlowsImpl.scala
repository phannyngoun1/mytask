...
import com.dream.common.Protocol.{CmdResponseFailed, DefaultTaskPerformCmdResponse, TaskPerformCmdResponse}
...
class ProcessInstanceAggregateFlowsImpl(aggregateRef: ActorRef) extends ProcessInstanceAggregateFlows {
  ...
  override def performTask: Flow[Protocol.PerformTaskCmdReq, Protocol.PerformTaskCmdRes, NotUsed] =
    Flow[Protocol.PerformTaskCmdReq]
      .mapAsync(1)(aggregateRef ? _)
      .map {
        case res: TaskPerformCmdResponse => Protocol.PerformTaskSuccess(res.activityId)
    ...
    Flow[Protocol.GetTaskCmdReq]
      ...
      .map {
        case GetTaskCmdRes(pInstId, participantId, flowId, task) =>
          ...
          Protocol.GetTaskCmdSuccess(TaskDto(task.id, pInstId, participantId, flowId, task.activity, Map.empty, task.active,
